	...
	public String SendResponse(Request request) throws Exception {
		...
		//Coumba's part
				...
				else if(request_name.equals("between_18_22"))
				{
					...
					op = connection.createStatement().executeUpdate(" UPDATE Windows SET status = 'Milieu', temperature = "+(Integer)data_loading.get("temperature")+" WHERE id_windows = "+(Integer)data_loading.get("id_windows")+" ");
					List<Map> update=new ArrayList<Map>();
					logger.info(op+" "+op);
					Map<String,Object> hm=new HashMap<String,Object>();
					if(op>0)
		...
		// Ines's part
			...
			Map data_loading = (Map) request.getData();
			ResultSet rs1 = connection.createStatement().executeQuery(
					"select w.level from lighting AS w  where  w.id_gs=" + (Integer) data_loading.get("id_gs"));
